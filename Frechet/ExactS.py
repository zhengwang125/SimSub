...
for i in range(len(traj_C)):
    print('-----' +str(i)+'-----')
    print(DIS.FRECHET(traj_C[sp:i+1], traj_Q)) #input prefix traj of traj_C
    print(DIS.FRECHET(traj_C[sp:i+1][::-1], traj_Q[::-1]))
'''
...
def ExactS(traj_c, traj_q):
    ...
    for i in range(N):
        ...
        for j in range(i, N):
            temp = DIS.FRECHET(traj_c[i:j+1], traj_q)
            #temp = similaritymeasures.frechet(traj_c[i:j+1], traj_q)[0]
