...
class Subtraj():
    ...
    def reset(self, episode, label='E'):
        ...
        whole = self.DIS_R.FRECHET(self.cand_train_data[episode][::-1],self.query_train_data[episode][::-1]) 
        self.sufsim = self.DIS_R.D[self.length-2,-1]#self.DIS_R.FRECHET(self.cand_train_data[episode][1:][::-1],self.query_train_data[episode][::-1])
        #print('reset',self.sufsim,self.DIS_R.D[self.length-2,-1])
        #self.DIS.FRECHET(self.cand_train_data[episode], self.query_train_data[episode])
        ...
        if label == 'T':
            ...
            self.presim_real = self.REWARD_DIS.FRECHET(self.cand_train_data[episode][self.split_point:1], self.query_train_data[episode])
    ...
    def step(self, episode, action, index, label='E'):
        if action == 0: #non-split 
            ...
            self.presim = self.DIS.FRECHET(self.cand_train_data[episode][self.split_point:(index+1)], self.query_train_data[episode], self.skip)
            self.sufsim = self.DIS_R.D[self.length-2-index,-1]#self.DIS_R.FRECHET(self.cand_train_data[episode][(index+1):][::-1],self.query_train_data[episode][::-1])
            ...
            if label == 'T':
                ...
                self.presim_real = self.REWARD_DIS.FRECHET(self.cand_train_data[episode][self.split_point:(index+1)], self.query_train_data[episode])
        ...
        if action > 1: #skip
            ...
            self.presim = self.DIS.FRECHET(self.cand_train_data[episode][self.split_point:(INX+1)], self.query_train_data[episode], self.skip)
            self.sufsim = self.DIS_R.D[self.length-2-INX,-1] #self.DIS_R.FRECHET(self.cand_train_data[episode][(INX+1):][::-1],self.query_train_data[episode][::-1])
            ...
            if label == 'T':
                last_subsim = self.subsim_real
                self.presim_real = self.REWARD_DIS.FRECHET(self.cand_train_data[episode][self.split_point:(INX+1)], self.query_train_data[episode])
                self.subsim_real = min(self.presim_real, self.sufsim, last_subsim)
                self.RW = last_subsim - self.subsim_real    
                #print(self.presim, self.presim_real)
    ...
    def output(self, index, episode, label='E'):
        ...
        if label == 'E':
            ...
            self.subsim_real = self.DIS.FRECHET(self.cand_train_data[episode][self.subtraj[0]:self.subtraj[1]+1], self.query_train_data[episode])
            return [self.subsim_real, self.subtraj]