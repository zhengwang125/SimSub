...
class DataOrderScaner():
    def __init__(self, srcfile, batch, seq_reverse=False, io_by_disk=False):
        self.srcfile = srcfile
        self.batch = batch
        self.srcdata = []
        self.start = 0
        self.seq_reverse = seq_reverse
        self.io_by_disk = io_by_disk
    def load(self, max_num_line=0, trajdata=None):
        if self.io_by_disk:
            num_line = 0
            with open(self.srcfile, 'r') as srcstream:
                for s in srcstream:
                    s = [int(x) for x in s.split()]
                    if self.seq_reverse:
                        self.srcdata.append(np.array(s, dtype=np.int32)[::-1])
                    else:
            ...
            for item in trajdata:
                self.srcdata.append(np.array(item, dtype=np.int32))
        # print(self.srcdata)
        self.size = len(self.srcdata)
        #print(self.srcdata)
        self.start = 0
