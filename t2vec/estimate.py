from Splitting_based import heuristic, adjust
...
def run_effective_rls_skip(elist):
    ...
    for e in elist:
        #print('e in elist', e)
        observation, steps, INX  = env.reset(e, 'E') #, INX 
        ...
        for index in range(1, steps):
            ...
            observation_, _, done, INX = env.step(e, action, index, 'E') #, done, INX 
...
def run_effective_rls(elist):
    ...
    relative_rank = []
    for e in elist:
        observation, steps = env.reset(e, 'E') #, INX 
        total = (steps) * (steps + 1) / 2
...
if __name__ == '__main__':
    ...
    '''
    #Be careful, do not overlap the SUBSIM_TEST, SUBSIM_RANK if you have generated (by dump), too time-consuming for groundtruth
    SUBSIM_TEST = []
    SUBSIM_RANK = []
    for i in elist:
